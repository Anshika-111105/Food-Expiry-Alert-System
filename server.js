...
/* ------------------------ Helpers ------------------------ */
const PHONE_REGEX = /^\+\d{1,3}\d{10}$/; // +countryCode + 10 digits
...
function ymd(s) {
  return new Date(s).toISOString().split("T")[0]; // YYYY-MM-DD
}
...
// multi-line rich body: name + (category) + date + time-left
...
const {
  TWILIO_ACCOUNT_SID,
  TWILIO_AUTH_TOKEN,
  TWILIO_WHATSAPP_FROM,     // whatsapp:+14155238886 (sandbox) or your WA-enabled Twilio number
  TWILIO_WA_TEMPLATE_SID,   // optional Business-Initiated template SID
  TWILIO_SMS_FROM,          // optional SMS fallback number (Twilio SMS-capable)
} = process.env;
...
async function sendWhatsAppFreeform(phone, body) {
  // Requires 24h session (user recently messaged sandbox join code) for free-form
  return twilioClient.messages.create({
    from: TWILIO_WHATSAPP_FROM, // 'whatsapp:+14155238886'
    to: `whatsapp:${phone}`,    // 'whatsapp:+9195...'
    body,
  });
}
...
async function sendSmsFallback(phone, body) {
  ...
  return twilioClient.messages.create({
    ...
    to: phone, // E.164, +<country><number>
    body,
  });
}

/** Orchestrator: WA free-form → WA template → SMS */
async function sendExpiryAlert(phone, textForUser, templateVarsIfNeeded) {
  ...
  // 2) WhatsApp template (Business-initiated)
  try {
...
/* ------------------------ Per-user notify route ------------------------ */
/** GET /notify/user/:user_id
 *  Sends an alert ONLY for this user's items that expire within 2 days.
 */
app.get("/notify/user/:user_id", (req, res) => {
  const user_id = parseInt(req.params.user_id, 10);
  if (!user_id) return res.status(400).send("Missing user_id");

  // get user's phone + name
...
/* ------------------------ Cron: Expiry Check + WA/SMS ------------------------ */
// Runs daily at 08:00 local time (existing 'expiring soon' job)
cron.schedule("0 8 * * *", () => {
  console.log("🕗 Checking for items expiring soon and sending alerts...");
  ...
  db.query(query, async (err, results) => {
    ...
    for (const [uid, payload] of Object.entries(grouped)) {
      ...
      console.log(`📣 Sending alert for User ${uid} (${phone}) for ${items.length} item(s).`);
      await sendExpiryAlert(phone, text, templateVars);
    }
  });
});

/* ------------------------ New Cron: Daily "expiring TODAY" job at 13:25 (1:25 PM) ------------------------ */
/*
  Purpose:
  - At 13:25 (Asia/Kolkata) every day, for each user:
    - If user has items where expiry_date = CURDATE(), send them a message listing today's expiring items.
    - If user has no items expiring today, send a short "No food is expiring today" message.
  Note: This is separate from the "expiring soon" job above.
*/
cron.schedule(
  "25 13 * * *",
  async () => {
    console.log("🕜 Running daily 'expiring TODAY' job at 13:25 (IST).");
    ...
    db.query("SELECT id, name, phone FROM users", async (errUsers, users) => {
      ...
      for (const user of users) {
        try {
          ...
          // 2) fetch items expiring TODAY for this user
          const qItemsToday = `
            SELECT item_name, category, expiry_date
            FROM food_items
            WHERE user_id = ?
          ...
          if (!itemsToday || itemsToday.length === 0) {
            ...
            console.log(`(Daily-13:25) sending "none expiring" to ${user.phone}`);
            const now = new Date();
            const templateVars = {
              "1": now.toLocaleDateString("en-IN"),
              "2": now.toLocaleTimeString("en-IN", { hour: "numeric", minute: "2-digit" }),
          ...
          // items found → send list
          const text = buildAlertText(itemsToday, user.name || "");
          console.log(`(Daily-13:25) sending TODAY-expiring alert to ${user.phone} for ${itemsToday.length} item(s).`);
          const now = new Date();
          const templateVars = {
            "1": now.toLocaleDateString("en-IN"),
            "2": now.toLocaleTimeString("en-IN", { hour: "numeric", minute: "2-digit" }),
...
/* ------------------------ Quick test route ------------------------ */
/** Still useful for sanity: hits Akriti's number with a dummy body. */
